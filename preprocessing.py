...
def cat_intersection_ids(case_path, label_path):
    ...
    filenames = set(os.listdir(case_path + '/FJMU_GBM_152_beta'))
...
def processing_interactions(intersected_ids, label_path, case_path):
    ...
    for case_id in intersected_ids:
        ...
        source_folder = os.path.join(case_path + '/FJMU_GBM_152_beta', case_id)
...
def achieve_img_and_mask_path(case_path):
    """
    Find the image and mask file paths in the 'merge' folders.
    ...
    for coded_folder in tqdm(coded_folders):
        merge_folder_path = os.path.join(coded_folder, 'merge')

        # 在merge文件夹中搜索image和mask文件
        image_files = glob(os.path.join(merge_folder_path, 'image*.nii.gz'))
        mask_files = glob(os.path.join(merge_folder_path, 'mask*.nii.gz'))

        # 如果找到了图像和掩模文件
        if image_files and mask_files:
            image_path.extend(image_files)
            mask_path.extend(mask_files)
        else:
            missing_folders.append(coded_folder)  # 添加缺失文件的文件夹路径

    return image_path, mask_path, missing_folders
...
print("missing_folders:", missing_folders)
